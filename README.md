...
Supabase·Redis는 단위 테스트에서 mock 처리하고, 핵심 도메인 로직(상태 머신, 게이팅, ASS 빌더, SRT 파싱, CC 사운드 이벤트 매핑·병합, 자막 스타일 추천, 섬네일 채점·랭킹, 알림 라우팅)을 결정적으로 검증합니다. 워커 CC 파이프라인은 실영상 headed E2E로도 검증했습니다.
